...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tile_status_dict = {}; # Dictionary where keys are tile bottom left corners and values are amount of dirt
        for i in range(width):
        	for j in range(height):
        		self.tile_status_dict[(i,j)] = dirt_amount;
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        current_dirt_amount = self.tile_status_dict[(tile_x, tile_y)];
        ...
        self.tile_status_dict[(tile_x, tile_y)] = new_dirt_amount;
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        if self.tile_status_dict[(m,n)] == 0:
        	return True
    ...
    def get_num_cleaned_tiles(self):
        ...
        for key in self.tile_status_dict.keys():
        	if self.tile_status_dict[key] == 0:
        		count += 1;
    ...
    def is_position_in_room(self, pos):
        ...
        test_x = math.floor(pos.get_x());
        test_y = math.floor(pos.get_y());
        
        if test_x+1 > self.width or test_x < 0 or test_y+1 > self.height or test_y < 0:
        	return False
    ...
    def get_dirt_amount(self, m, n):
        ...
        Returns: an integer
        """
        return self.tile_status_dict[(m,n)]
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        return self.is_tile_furnished(test_x,test_y)
    ...
    def get_random_position(self):
        ...
        while True:
            random_x = random.uniform(0,self.width);
            random_y = random.uniform(0,self.height);
            P = Position(random_x,random_y);

            if self.is_position_valid(P):
                return P
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        robot_capacity = self.capacity;

        test_pos = current_pos.get_new_position(current_dir,robot_speed);

        if self.room.is_position_valid(test_pos):
            self.set_robot_position(test_pos);
            self.room.clean_tile_at_position(test_pos,robot_capacity);
        else:
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial in range(num_trials):
        ...
        #Set up the room and robots used in each simulation
        sim_room = EmptyRoom(width,height,dirt_amount);

        robot_dict = {};
        for robot in range(num_robots):
            robot_dict[robot] = robot_type(sim_room,speed,capacity);
            robot_dict[robot].set_robot_position(sim_room.get_random_position());

        #Initialize Time Step Counter before each simulation
        timestep_counter = 0;
        min_coverage_reached = False;

        #Simulation run
        while not min_coverage_reached:
            timestep_counter += 1;

            for robot in range(num_robots):
                robot_dict[robot].update_position_and_clean();

            tiles_cleaned = sim_room.get_num_cleaned_tiles();
            pct_tiles_cleaned = float(tiles_cleaned/num_tiles);

            if pct_tiles_cleaned >= min_coverage:
                min_coverage_reached = True;
                timestep_list.append(timestep_counter);


    avg = float((sum(timestep_list))/num_trials);
