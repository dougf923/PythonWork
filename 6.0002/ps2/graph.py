...
class Digraph(object):
    ...
    def add_edge(self, edge):
        ...
        if self.has_node(edge.get_source()) and self.has_node(edge.get_destination()):
            try:
                a = self.edges[edge.get_source()];
                if edge not in a:
                    a.append(edge);
                    self.edges[edge.get_source()] = a;
            except KeyError:
                self.edges[edge.get_source()] = [edge];

				
            try:
                b = self.edges[edge.get_destination()];
                if edge not in b:
                    b.append(edge);
                    self.edges[edge.get_destination()] = b;
            except KeyError:
